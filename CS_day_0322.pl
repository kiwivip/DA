...
for ( 1 .. $time_step + 1 )
{
	...
	my $sth_auth = $dbh_v506_user -> prepare("
											...
											WHERE
											r.authType = 20 and r.eventTime between $timestamp_l and $timestamp_r
										") ;
	$sth_auth -> execute();
...
for ( 1 .. $time_step + 1 )
{
	...
	{
			my $accountId   = $ref -> {accountId} ;
            my $avatarsPath = $ref -> {avatarsPath} ;
            $avatarsPath =~ s/\s+$//g ;
			next if exists $paths_d{$avatarsPath} ;
